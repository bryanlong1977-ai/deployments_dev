...
resource "azurerm_subnet" "subnets" {
  ...
  dynamic "delegation" {
    for_each = each.key == var.snet_inbound_idm_cus_01_subnet_name ? [1] : []
    content {
  ...
  dynamic "delegation" {
    for_each = each.key == var.snet_outbound_idm_cus_01_subnet_name ? [1] : []
    content {
      name = "Microsoft.Network.dnsResolvers"
      service_delegation {
        name    = "Microsoft.Network/dnsResolvers"
...
resource "azurerm_private_dns_resolver_inbound_endpoint" "this" {
  ...
  ip_configurations {
    ...
    subnet_id                    = azurerm_subnet.subnets[var.snet_inbound_idm_cus_01_subnet_name].id
  }
}
...
resource "azurerm_private_dns_resolver_outbound_endpoint" "this" {
  ...
  private_dns_resolver_id = azurerm_private_dns_resolver.this.id
  location                = var.region
  subnet_id               = azurerm_subnet.subnets[var.snet_outbound_idm_cus_01_subnet_name].id
  tags                    = var.tags
}
...
resource "azurerm_private_dns_resolver_dns_forwarding_ruleset" "this" {
  name                                       = "dnsrs-identity-prd-cus-01"
  resource_group_name                        = azurerm_resource_group.dns.name
  location                                   = var.region
