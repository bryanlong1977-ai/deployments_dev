...
# Network Resource Group for Identity VNet
...
# DNS Resource Group for Private DNS Resolver and Zones
...
resource "azurerm_subnet" "ib_mgmt" {
...
resource "azurerm_subnet" "ib_lan1" {
...
#------------------------------------------------------------------------------
# Private DNS Zone VNet Links - Link to Identity VNet
#------------------------------------------------------------------------------

resource "azurerm_private_dns_zone_virtual_network_link" "identity" {
  for_each              = azurerm_private_dns_zone.zones
  name                  = "link-${var.vnet_name}-${replace(each.key, ".", "-")}"
  resource_group_name   = azurerm_resource_group.dns.name
...
#------------------------------------------------------------------------------
# Private DNS Zone VNet Links - Link to Hub VNet (Connectivity)
#------------------------------------------------------------------------------

resource "azurerm_private_dns_zone_virtual_network_link" "hub" {
  for_each              = azurerm_private_dns_zone.zones
  name                  = "link-${data.terraform_remote_state.connectivity_network_1.outputs.vnet_name}-${replace(each.key, ".", "-")}"
  resource_group_name   = azurerm_resource_group.dns.name
  private_dns_zone_name = each.value.name
...
#------------------------------------------------------------------------------
# DNS Forwarding Ruleset VNet Link - Identity VNet
#------------------------------------------------------------------------------
...
resource "azurerm_virtual_network_peering" "identity_to_hub" {
  name                         = "peer-${var.vnet_name}-to-${data.terraform_remote_state.connectivity_network_1.outputs.vnet_name}"
  resource_group_name          = azurerm_resource_group.network.name
  virtual_network_name         = azurerm_virtual_network.identity.name
...
#------------------------------------------------------------------------------
# VNet Peering - Hub to Identity (created in Connectivity subscription)
#------------------------------------------------------------------------------

resource "azurerm_virtual_network_peering" "hub_to_identity" {
  provider                     = azurerm.connectivity
  name                         = "peer-${data.terraform_remote_state.connectivity_network_1.outputs.vnet_name}-to-${var.vnet_name}"
